  ...
  export default function ReservarHabitacion() {
    ...
    const [errorModal, setErrorModal] = useState({ visible: false, mensaje: "" });
    ...
    const handleAgregarSeleccion = () => {
      ...
      if (seleccionadaReserva.length < 2) {
        setErrorModal({
            visible: true,
            mensaje: "La estancia mínima es de 2 días (una noche). Por favor selecciona fecha de entrada y salida."
        });
        return;
      }
        ...
        {errorModal.visible && (
          <div className={styles.modalOverlay}>
            <div className={styles.modalContent}>
              ...
              <h3 className={styles.modalTitulo}>No disponible</h3>
              <p className={styles.modalMensaje}>{errorModal.mensaje}</p>
              
              <button 
                className={styles.btnModalCerrar}
                onClick={() => setErrorModal({ visible: false, mensaje: "" })}
              >
                Entendido
              </button>
            </div>
