...
export default function OcuparHabitacion() {
  ...
  const seleccionarTitular = (huesped) => {
    const hoy = new Date();
    const nacimiento = new Date(huesped.fechaNacimiento);
    ...
    if (edad < 18) {
      mostrarError(
        `El huésped titular (${huesped.nombre} ${huesped.apellido}) debe ser mayor de edad (Tiene ${edad} años).`
      );
      return;
    }
  ...
  const handleContinuar = () => {
    ...
    seleccionadaReserva.forEach((item) => {
      ...
      if (
        ["OCUPADA", "MANTENIMIENTO", "FUERA_DE_SERVICIO"].includes(estadoCelda)
      )
        hayBloqueo = true;
  ...
  const finalizarOcupacion = async () => {
    ...
    try {
      ...
      setModalConfig({
        ...
        acciones: [
          {
            texto: "Ocupar otra habitación",
            estilo: "aceptar",
            onClick: () => {
              setHabitaciones((prev) =>
                prev.map((h) => {
                  const esOcupada = reservasAcumuladas.some(
                    ([fecha, id]) => fecha === h.fecha && id === h.id
                  );
                  return esOcupada ? { ...h, estado: "OCUPADA" } : h;
                })
              );
              cerrarModal();
              setReservasAcumuladas([]);
  ...
  const mostrarLista = () => {
    ...
    const deshabilitarConfirmar =
      !huespedSeleccionado || cargando || confirmando;
        ...
        {!mostrandoFormularioHuesped ? (
          ...
          <div className={styles.formularioReservaContainer}>
            <div className={styles.cardFormulario}>
                        ...
                        <>
                          ...
                          >
                            <div
                              ...
                              onClick={() =>
                                !confirmando &&
                                manejarOrdenamientoHuesped("apellido")
                              }
                            >
                              Apellido
                            ...
                            <div
                              ...
                              onClick={() =>
                                !confirmando &&
                                manejarOrdenamientoHuesped("nombre")
                              }
                            >
                              Nombre
                          ...
                          <div className={styles.contenidoListaContainer}>
                            {huespedesOrdenados.map((h) => (
                              <div
                                ...
                                onClick={() =>
                                  !confirmando && seleccionarTitular(h)
                                }
                              >
                ...
                <>
                  ...
                  <div style={{ display: "flex", justifyContent: "center" }}>
                    ...
                    >
                      Titular:{" "}
                      <b>
                        {huespedSeleccionado?.apellido?.toUpperCase()}{" "}
                        {huespedSeleccionado?.nombre?.toUpperCase()}
                      </b>{" "}
                      <br />
                      <small>
                  ...
                  <form onSubmit={buscarPersonaEnBD} id="formBuscarHuesped">
                    <div className={styles.formReservaInputsContainer}>
                      <div className={styles.filaInputs}></div>
                      <div className={styles.filaInputs}>
                        <div className={styles.inputContainer}>
                          <label>Tipo Doc.</label>
                          <select
                      ...
                      {Object.keys(acompananteEncontrado).length > 0 && (
                        ...
                        >
                          Encontrado: {acompananteEncontrado.nombre}{" "}
                          {acompananteEncontrado.apellido}
                        </div>
                      )}
              ...
              >
                <h3>
                  {!huespedSeleccionado && acompananteEncontrado == {}
                    ? "Seleccione el huésped titular"
                    : ""}
                </h3>
